...
// Component for download icon with progress
function DownloadIcon({ 
  ...
  if (isDownloaded) {
    return <CheckCircle2 className="w-4 h-4 fill-current" />;
  }
...
export default function Home() {
  ...
  const getDownloadInfo = (surahNumber: number, reciterId: string) => {
    return downloads?.find(d => d.surahNumber === surahNumber && d.reciterId === reciterId);
  };

  const isSurahDownloaded = (surahNumber: number, reciterId: string) => {
    const download = getDownloadInfo(surahNumber, reciterId);
    return download?.status === "completed";
  };

  const isSurahDownloading = (surahNumber: number, reciterId: string) => {
    const download = getDownloadInfo(surahNumber, reciterId);
    return download?.status === "downloading";
  };

  const getDownloadProgress = (surahNumber: number, reciterId: string): number => {
    const download = getDownloadInfo(surahNumber, reciterId);
                ...
                <CardContent className="p-3 flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <Button
                      ...
                      className={cn(
                        "h-8 w-8",
                        isSurahDownloaded(surah.number, String(selectedReciter.id)) ? "text-green-500" : "text-muted-foreground",
                        isSurahDownloading(surah.number, String(selectedReciter.id)) && "text-blue-500"
                      )}
                      onClick={() => handleAction("Download", surah)}
                    >
                      <DownloadIcon 
                        isDownloaded={isSurahDownloaded(surah.number, String(selectedReciter.id))}
                        isDownloading={isSurahDownloading(surah.number, String(selectedReciter.id))}
                        progress={getDownloadProgress(surah.number, String(selectedReciter.id))}
                      />
                    </Button>
                  </div>
                </CardContent>
