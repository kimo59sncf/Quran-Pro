  ...
  return (
    <div className="min-h-screen bg-background pb-32">
      ...
      <ScrollArea className="px-4 py-6 space-y-8">
        ...
        {/* Continue Reading Section (Latest Bookmark) */}
        {bookmarks && bookmarks.length > 0 && (
          <div className="mb-8">
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
              {(() => {
                // Sort bookmarks by createdAt in descending order and take the latest one
                const latestBookmark = [...bookmarks].sort((a, b) =>
                  new Date(b.createdAt || new Date()).getTime() - new Date(a.createdAt || new Date()).getTime()
                )[0];

                return (
                  <Link key={latestBookmark.id} href={`/surah/${latestBookmark.surahNumber}`} className="shrink-0">
                    <Card className="w-40 hover:border-primary transition-colors cursor-pointer bg-card/50">
                      <CardContent className="p-4">
                        <div className="text-xs text-muted-foreground mb-1">Surah {latestBookmark.surahNumber}</div>
                        <div className="font-bold text-lg mb-1 truncate">
                          {surahs?.find(s => s.number === latestBookmark.surahNumber)?.englishName}
                        </div>
                        <div className="text-xs text-primary">Ayah {latestBookmark.ayahNumber}</div>
                      </CardContent>
                    </Card>
                  </Link>
                );
